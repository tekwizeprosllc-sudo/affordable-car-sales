...
export async function ensureVehicleSchema() {
  if (!vehicleSchemaReady) {
    vehicleSchemaReady = (async () => {
      ...
      await query('ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS doors INTEGER')
      await query('ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS cylinders INTEGER')
      await query('ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS purchase_cost INTEGER')
      await query('ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS reconditioning_cost INTEGER')
      await query('ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS dealer_fees INTEGER')
      await query('ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS warranty_cost INTEGER')
      await query('ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS interest_rate NUMERIC(5,2)')
