...
export const VEHICLE_STATUSES = ['live', 'pending', 'sold', 'unknown']

// 'live' predates this label set and means "available" — kept as the stored
// value so existing rows and the public-site filter (`status !== 'sold'`)
// don't need a data migration.
export const VEHICLE_STATUS_LABELS = { live: 'Available', pending: 'Pending', sold: 'Sold', unknown: 'Unknown' }

// WCAG AA (4.5:1) for white text at the small badge sizes these render at —
// see the note by STATUS_COLORS in lib/leads.js for how these were picked.
export const VEHICLE_STATUS_COLORS = { live: '#1A7A4C', pending: '#9C6300', sold: '#5C6066', unknown: undefined }
...
function toVehicle(row) {
  return {
    ...
    dealerFees: row.dealer_fees,
    warrantyCost: row.warranty_cost,
    interestRate: row.interest_rate == null ? null : Number(row.interest_rate),
    grossProfit: row.price == null
      ? null
      : row.price - (row.purchase_cost || 0) - (row.reconditioning_cost || 0) - (row.dealer_fees || 0) - (row.warranty_cost || 0),
...
export async function createVehicle(input) {
  ...
  const rows = await query(
    `INSERT INTO vehicles (year, make, model, trim, price, mileage, body_type, drive,
       engine, trans, fuel_type, doors, cylinders, color, vin, stock, purchase_cost,
       reconditioning_cost, dealer_fees, warranty_cost, interest_rate, description,
       badges, status, photos)
    ...
    [
      ...
      input.reconditioningCost || null,
      input.dealerFees || null,
      input.warrantyCost || null,
